...
func (s *RunStore) CreateRun(r *model.RunDetail) (*model.RunDetail, error) {
	if r.StorageState == "" {
		r.StorageState = apiv2beta1.Run_STORAGESTATE_AVAILABLE.String()
	} else if r.StorageState != apiv2beta1.Run_STORAGESTATE_AVAILABLE.String() &&
		r.StorageState != apiv2beta1.Run_STORAGESTATE_ARCHIVED.String() {
...
func (s *RunStore) UnarchiveRun(runId string) error {
	sql, args, err := sq.
		Update("run_details").
		SetMap(sq.Eq{
			"StorageState": apiv2beta1.Run_STORAGESTATE_AVAILABLE.String(),
		}).
		Where(sq.Eq{"UUID": runId}).
		ToSql()
