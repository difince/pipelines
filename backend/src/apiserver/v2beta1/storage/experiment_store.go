...
func (s *ExperimentStore) scanRows(rows *sql.Rows) ([]*model.Experiment, error) {
	...
	for rows.Next() {
		...
		if experiment.StorageState == "" {
			experiment.StorageState = apiv2beta1.Experiment_STORAGESTATE_AVAILABLE.String()
		}
...
func (s *ExperimentStore) CreateExperiment(experiment *model.Experiment) (*model.Experiment, error) {
	...
	if newExperiment.StorageState == "" {
		...
		newExperiment.StorageState = apiv2beta1.Experiment_STORAGESTATE_AVAILABLE.String()
	} else if newExperiment.StorageState != apiv2beta1.Experiment_STORAGESTATE_AVAILABLE.String() &&
		newExperiment.StorageState != apiv2beta1.Experiment_STORAGESTATE_ARCHIVED.String() {
...
func (s *ExperimentStore) ArchiveExperiment(expId string) error {
	...
	sql, args, err := sq.
		Update("experiments").
		SetMap(sq.Eq{
			"StorageState": apiv2beta1.Experiment_STORAGESTATE_ARCHIVED.String(),
		}).
	...
	updateRunsSql, updateRunsArgs, err := sq.
		Update("run_details").
		SetMap(sq.Eq{
			"StorageState": apiv2beta1.Run_STORAGESTATE_ARCHIVED.String(),
		}).
	...
	updateRunsWithExperimentUUIDSql, updateRunsWithExperimentUUIDArgs, err := sq.
		...
		Where(sq.NotEq{"StorageState": apiv2beta1.Run_STORAGESTATE_ARCHIVED.String()}).
		ToSql()
	if err != nil {
		return util.NewInternalServerError(err,
			"Failed to create query to archive the runs in an experiment %s. error: '%v'", expId, err.Error())
...
func (s *ExperimentStore) UnarchiveExperiment(expId string) error {
	...
	sql, args, err := sq.
		Update("experiments").
		SetMap(sq.Eq{
			"StorageState": apiv2beta1.Experiment_STORAGESTATE_AVAILABLE.String(),
		}).
		Where(sq.Eq{"UUID": expId}).
		ToSql()
	if err != nil {
